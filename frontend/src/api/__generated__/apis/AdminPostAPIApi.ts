...
export class AdminPostAPIApi extends runtime.BaseAPI {
  ...
  async deletePostRaw(
  ...
  ): Promise<runtime.ApiResponse<RsDataLong>> {
    ...
    const response = await this.request(
      {
        path: `/api/admin/posts/{id}`.replace(
          `{${"id"}}`,
   ...
   * 게시글 목록 조회 (관리자용)
   */
  async getPostListRaw(
    requestParameters: GetPostListRequest,
    initOverrides?: RequestInit | runtime.InitOverrideFunction,
  ...
  async updatePostRaw(
    ...
    initOverrides?: RequestInit | runtime.InitOverrideFunction,
  ): Promise<runtime.ApiResponse<RsDataPostUpdateResponse>> {
    if (requestParameters["id"] == null) {
      throw new runtime.RequiredError(
        "id",
    ...
    return new runtime.JSONApiResponse(response, (jsonValue) =>
      RsDataPostUpdateResponseFromJSON(jsonValue),
    )
  }
  ...
  async updatePost(
    requestParameters: UpdatePostRequest,
    initOverrides?: RequestInit | runtime.InitOverrideFunction,
  ): Promise<RsDataPostUpdateResponse> {
