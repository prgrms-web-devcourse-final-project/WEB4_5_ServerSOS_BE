...
import type { RedirectView } from "../models/index"
import { RedirectViewFromJSON, RedirectViewToJSON } from "../models/index"
...
export class KakaoAPIApi extends runtime.BaseAPI {
   ...
   */
  async login1Raw(
    requestParameters: Login1Request,
    ...
    return new runtime.JSONApiResponse(response, (jsonValue) =>
      RedirectViewFromJSON(jsonValue),
    )
  }
  ...
  /**
   */
  async redirectToKakaoLoginRaw(
    requestParameters: RedirectToKakaoLoginRequest,
    initOverrides?: RequestInit | runtime.InitOverrideFunction,
  ): Promise<runtime.ApiResponse<RedirectView>> {
    if (requestParameters["origin"] == null) {
      throw new runtime.RequiredError(
  ...
  async redirectToKakaoLogin(
    ...
    const response = await this.redirectToKakaoLoginRaw(
      requestParameters,
      initOverrides,
    )
    return await response.value()
